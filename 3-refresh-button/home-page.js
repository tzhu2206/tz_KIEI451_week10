...
exports.pageLoaded = async function (args) {
  ...
  // listen for "tap" event on refreshButton
  let refreshButton = page.getViewById('refreshPosts')
  refreshButton.addEventListener('tap', async function (event) {
    console.log('tapped!')
    // empty the postData array
    postsData.length = 0
